function criarConta() {
    ...
    if (!usuario.nome || !usuario.sobrenome || !usuario.email || !usuario.senha) {
        ...
        if (usuario.email.includes("@") && usuario.email.includes(".com")) {
            ...
            let testeEmail = usuarios.find(teste => (teste.email || "").toLowerCase() === usuario.email);
            ...
            if (testeEmail && testeEmail != usuario) {
                ...
                if (usuario.senha.length > 4 && numerosTeste && caraterestersTeste) {
                    ...
                    alert("A senha precisa ter mais que quatro caracteres, conter pelo menos um número, uma letra e um desses caracteres: ! @ $ %");
                }
            }
...
function entrarConta() {
    ...
    let procurarUsuario = usuarios.find(usuarioTeste => usuarioTeste.email === email);
...
function nomeBemvindo() {
    ...
    paginaCadastro.textContent = `Bem Vindo! ${usuarioLogado.nome} ${usuarioLogado.sobrenome}, é um prazer ter você aqui!`;
...
function mudarEmail() {
    ...
    if (!velhoEmail || !novoEmail) {
        ...
        let usuario = usuarios.find(usuarioTeste => (usuarioTeste.email || "").toLowerCase() === velhoEmail);
        ...
        if (!usuario) {
            ...
            let testeEmail = usuarios.find(usuarioTeste => (usuarioTeste.email || "").toLowerCase() === novoEmail);
...
function nomesFilmes() {
    ...
    usuarioLogado.forEach(usuario => {
        ...
        pFilmes.innerHTML = `
      <div class="work-text">
        <p><b>Nome do Filme:</b> ${usuario.nomeobra}</p>
        <p><b>Data:</b> ${usuario.tempoDia}/${usuario.tempoMes}/${usuario.tempoAno}</p>
      </div>
    `;
...
function recuperarSenha() {
    ...
    let eMail = document.getElementById("nova__senha_e-mail").value.trim().toLowerCase();

    let usuarios = JSON.parse(localStorage.getItem("usuarios")) || [];

    let procurarUsuario = usuarios.find(usuario => usuario.email === eMail);
    ...
    if (!eMail || !novaSenha) {
        ...
        if (!procurarUsuario) {
            ...
            procurarUsuario.senha = novaSenha;
